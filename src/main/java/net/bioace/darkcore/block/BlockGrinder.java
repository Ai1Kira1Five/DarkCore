...
/**
 * Created by BIOACE on 05.04.2015.
 */
public class BlockGrinder extends BlockContainer {
    ...
    public void registerBlockIcons(IIconRegister iconRegister){
        this.blockIcon = iconRegister.registerIcon(DarkCore.MOD_ID + ":" + "GrinderSide");
        this.iconFront = iconRegister.registerIcon(DarkCore.MOD_ID + ":" +(this.isActive ? "GrinderFrontOn" : "GrinderFrontOff"));
        this.iconTop = iconRegister.registerIcon(DarkCore.MOD_ID + ":" + "GrinderTop");
    ...
    public Item getItemDropped(int i, Random random, int j){
        return Item.getItemFromBlock(Content.BlockGrinderIdle);
    }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ){
        if(!world.isRemote){
            player.openGui(DarkCore.instance, 0, world, x, y, z);
        }
        return true;
    }
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entityplayer, ItemStack itemstack){
        ...
        if(itemstack.hasDisplayName()){
            ((TEGrinder)world.getTileEntity(x, y, z)).setGuiDisplayName(itemstack.getDisplayName());
        }
    }
    ...
    public static void updateAlabasterOvenBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord) {
        ...
        if (active){
            worldObj.setBlock(xCoord, yCoord, zCoord, Content.BlockGrinderActive);
        }else{
            worldObj.setBlock(xCoord, yCoord, zCoord, Content.BlockGrinderIdle);
        }
    ...
    public void breakBlock(World world, int x, int y, int z, Block oldblock, int oldMetadata){
        if (!keepInventory){
            TEGrinder tileentity = (TEGrinder) world.getTileEntity(x, y, z);
